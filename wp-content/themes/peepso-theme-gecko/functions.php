...
function gecko_load_theme_setup() {
    ...
    // Language
    load_theme_textdomain( 'peepso-theme-gecko', get_template_directory() . '/language' );
    ...
    if (! is_404() || gecko_is_shop()) {
        if ( class_exists('PeepSo') ) {
            ...
            foreach ($shortcodes as $sc => $method) {
                if ($post instanceof WP_Post && (stristr($post->post_content, "[$sc") || strpos($post->post_content, '<!-- wp:peepso') !== FALSE)) {
                    if($class == 1) {
