...
class PeepSoMessagesModel
{
	...
	public function get_messages($type, $user_id, $limit = 40, $offset = 0, $query = NULL, $unread_only = 0)
	{
		...
		// build main query
		$sql = 'SELECT `mrec`.* FROM  `'. $wpdb->prefix . PeepSoMessageRecipients::TABLE .'` `mrec` ';
		$sql .= ' LEFT JOIN `' . $wpdb->posts . '` `posts` ON `mrec`.`mrec_msg_id` = `posts`.`ID`';

		if (FALSE === empty($query)) {
			// left join with users table
			$sql .= ' LEFT JOIN `' . $wpdb->users . '` `users` ON `mrec`.`mrec_user_id` = `users`.`ID`';

			// left join with refined usermeta table
		...
		// not deleted
		$sql .= ' WHERE `mrec`.`mrec_deleted` <> 1';

		// select all message id belongs to user
		$sql .= ' AND `mrec`.`mrec_msg_id` IN (SELECT `mrec_msg_id` FROM `'. $wpdb->prefix . PeepSoMessageRecipients::TABLE .'`  WHERE `mrec_user_id` =' . intval($user_id) . ')';

		// select only specific post type
		$sql .=	' AND (`post_type` = "' . PeepSoMessagesPlugin::CPT_MESSAGE . '" OR `post_type` = "' . PeepSoMessagesPlugin::CPT_MESSAGE_INLINE_NOTICE . '")';

		// only shows message from existing users
		...
		// if unread only
		if ($unread_only) {
			$sql .= ' AND `mrec`.`mrec_viewed` = 0 AND `mrec`.`mrec_user_id` = ' . intval($user_id);
		}
		...
		$sql .= ' ORDER BY `mrec`.`mrec_msg_id` DESC';

		// get ID's
		$sql = 'SELECT MAX(`tmp_table`.`mrec_msg_id`) FROM (' . $sql . ') `tmp_table` GROUP BY `tmp_table`.`mrec_parent_id`';
